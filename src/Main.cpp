...
#include "SecondaryLoop.h" //Secondary loop control for Core0
...
const char *PRIMARY_MQTT_CLIENT_NAME = "G86-INFO";    // Primary MQTT client name
const char *SECONDARY_MQTT_CLIENT_NAME = "G86-INFO2"; // Secondary MQTT client name
...
WiFiClient net2;  // wifi client for mqtt2
MQTTClient mqtt;  // primary mqtt connection
MQTTClient mqtt2; // seondary mqtt connection
...
void paramSave();
void MqttMessageReceivedPrimary(String &, String &);
void MqttMessageReceivedSecondary(String &, String &);
...
void setupMqtt(void)
{
  ...
  Serial.print("\nconnecting to MQTT Primary to " + String(Config.mqtt_server) + ":" + String(Config.mqtt_port));
  ...
  mqtt2.begin(Config.mqtt_server, net2);
  mqtt2.onMessage(MqttMessageReceivedSecondary);

  Serial.print("\nconnecting to MQTT Secondary to " + String(Config.mqtt_server) + ":" + String(Config.mqtt_port));
...
char curMessage[TXT_BUF_SIZE];
char newMessage[TXT_BUF_SIZE];
volatile char secondaryScreenMode[] = "WELCOME";
volatile bool newMessageAvailable2 = true;
volatile char newMessage2[TXT_BUF_SIZE];
...
MD_Menu::value_t *mnuValueRqst(MD_Menu::mnuId_t id, bool bGet)
{
  ...
  case 4: // ECU Values
    if (!bGet)
    {
      mqtt2.unsubscribe(MQTT_ECU_TOPIC + String(dataIndex2));
      mqtt2.unsubscribe(MQTT_GPS_TOPIC + String(dataIndex2));
    }
    ...
    else
    {
      ...
      // subscribe to chosen MQTT topic
      mqtt2.subscribe(MQTT_ECU_TOPIC + String(dataIndex2));
  ...
  case 5: // GPS Values
    ...
    else
    {
      ...
      // subscribe to chosen MQTT topic
      mqtt2.subscribe(MQTT_GPS_TOPIC + String(dataIndex2));
      strcpy(const_cast<char *>(secondaryScreenMode), "MQTT");
...
void MqttMessageReceivedPrimary(String &topic, String &payload)
{
...
void MqttMessageReceivedSecondary(String &topic, String &payload)
{

  // reverse string, as 7segment display expects it.
  reverseString(payload);
...
void setup()
{
  ...
  prefs.begin(PRIMARY_MQTT_CLIENT_NAME, false);
...
void loop()
{
  // starting MQTT processing loops
  mqtt.loop();
  mqtt2.loop();
